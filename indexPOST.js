...
app.post("/api/emailvalidation", (req, res) => {
  try {
    ...
    if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(req.body.email)) {
      ...
      return res.json({ success: false, error: "please enter valid email id" });
    }
  } catch (error) {
...
//authorisation concept vvvvvvimp
//logout

//----Authorization basics-----

// There are two types of Apis
// 1.Public Apis :that are freely accesible doesn't require login.E.g:landing Pages
// 2.Privtae APIs :that are only accesible after login.e.g:your profile

// private Apis works on the principle of cookies.
// whenever a user login to the server with correct credentials,server returns a token (ticket)
// Now this token is stored in cookies of browser till the time user log out
//
